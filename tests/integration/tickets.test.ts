...
describe("GET /tickets/types/:tickedTypeName", () => {
  it("should respond with status 401 if no token is given", async () => {
    const response = await server.get("/tickets/types/name");
  ...
  it("should respond with status 401 if given token is not valid", async () => {
    ...
    const response = await server.get("/tickets/types/name").set("Authorization", `Bearer ${token}`);
  ...
  describe("when token is valid", () => {
    it("should respond with empty array when no ticketType is found", async () => {
      const token = await generateValidToken();
    ...
    it("should respond with status 200 and with existing TicketType data", async () => {
      const token = await generateValidToken();

      const ticketType = await createTicketType();

      const response = await server.get(`/tickets/types/${ticketType.name}`).set("Authorization", `Bearer ${token}`);
